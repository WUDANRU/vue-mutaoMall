...
// 我们获取到了数据是不是数据加载完的时候，是的
// 我们是监听到我们的数据时就立马更新了(滚动条)
// 但是数据变化了，vue花一定的时间吧它渲染出来
// 如果渲染慢了，更新早了，怎么办
// 数据准备好了(数据改变了)并不能保证DOM渲染出来了
// 数据改变了,DOM并没渲染完，所以在this.$nextTick( ( )=>{ } )里面执行它


    watch: {
      data() {
        this.update(); // 我们是监听到我们的数据时就立马更新了
      }
    },
    created() {
      this.init();
    ...
    methods: {
      update() {
        this.$nextTick( ()=>{ this.$refs.swiper && this.$refs.swiper.swiper.update(); } )
        // this.$refs.swiper && this.$refs.swiper.swiper.update(); 原来是这样的
      },
